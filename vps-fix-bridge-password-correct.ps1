...
$CorrectBridgePassword = "MoQL_M-Loyi1fB3b9tKWew"
...
Write-Host "[1/4] .env dosyasinda dogru password guncelleniyor..." -ForegroundColor Cyan

# Password'ü escape et (özel karakterler için)
$EscapedPassword = $CorrectBridgePassword -replace '\$', '\$' -replace '`', '\`'
...
# Bridge logları kontrol
Write-Host ""
Write-Host "Bridge loglari (son authentication):" -ForegroundColor Cyan
$BridgeLogs = Invoke-VpsCmd "sudo journalctl -u protonmail-bridge -n 10 --no-pager | grep -i -E 'login|password|auth|error' | tail -5"
...
Write-Host "DOGRU PASSWORD GUNCELLENDI!" -ForegroundColor Green
Write-Host "========================================" -ForegroundColor Green
